...
var CACHE_TTL = { rdap: 3600, dns: 300, bootstrap: 86400 };
...
function cacheSet(key, value, ttlSeconds) {
  ...
  cache.set(key, { value, expires: Date.now() + ttlSeconds * 1e3, created: Date.now() });
}
__name(cacheSet, "cacheSet");
var rateLimits = /* @__PURE__ */ new Map();
var RATE_LIMIT = 20;
var RATE_WINDOW = 6e4;
var RATE_MAX_ENTRIES = 1e3;
function checkRateLimit(ip) {
  const now = Date.now();
  if (rateLimits.size > RATE_MAX_ENTRIES) {
    for (const [key, entry2] of rateLimits) {
      if (now - entry2.windowStart > RATE_WINDOW) {
...
function json(data, status = 200) {
  if (status === 200 && typeof data === "object" && !Array.isArray(data)) {
    data._upgrade = { note: "Upgrade for higher limits & priority support", url: "https://rapidapi.com/miccho27-5OJaGGbBiO/api/whois-domain-api/pricing" };
  }
...
__name(getRdapServer, "getRdapServer");
  ...
  if (data.entities) {
    for (const entity of data.entities) {
      if (entity.roles && entity.roles.includes("registrar")) {
        result.registrar = {
          name: entity.vcardArray ? extractVcardFn(entity.vcardArray) : entity.handle || null,
...
__name(parseRdapResponse, "parseRdapResponse");
...
__name(extractVcardFn, "extractVcardFn");
var DNS_TYPES = ["A", "AAAA", "MX", "NS", "TXT", "CNAME"];
