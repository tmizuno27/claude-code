"""
X (Twitter) 自動投稿スクリプト — sim-hikaku.online用
...
# sim-hikaku.online 向けの時間帯設定
SLOT_CONFIG = {
    "morning": {
        "categories": ["格安SIM節約術", "プラン比較"],
        "weights": [0.6, 0.4],
        "description": "朝の投稿。通勤中に読まれるスマホ料金の節約Tips・プラン比較",
    },
    "noon": {
        "categories": ["乗り換え体験談", "記事告知"],
        "weights": [0.5, 0.5],
        "description": "昼の投稿。実際の乗り換え体験や最新記事の紹介",
    },
    "evening": {
        "categories": ["キャンペーン情報", "用途別おすすめ"],
        "weights": [0.5, 0.5],
        "description": "夜の投稿。お得なキャンペーンや用途別SIMの選び方",
    },
}

SYSTEM_PROMPT = """あなたはX（Twitter）の投稿文を生成するアシスタントです。
以下のサイト情報とルールに従って、1つの投稿文を生成してください。

## サイト情報
- サイト: SIM比較オンライン（sim-hikaku.online）
- コンセプト: 用途別・悩み別で選ぶ格安SIM比較サイト
- ターゲット: スマホ料金を安くしたい人、格安SIMに興味がある人
...
5. 具体的な数字を入れる（月額料金・GB数・節約額など）
6. 実用的で役立つ情報を必ず含める
7. 自然な日本語で、親しみやすい語り口
8. エンゲージメントを高める工夫を必ず入れる：質問形（「まだ〜してない？」「月〜円払ってない？」）、驚きの数字、比較（ビフォーアフター）、あるある系
9. 記事告知カテゴリの場合、必ず記事URLを末尾に含める

## ハッシュタグ候補（トレンドに合わせて選択）
メイン: #格安SIM #スマホ料金 #通信費節約
サブ（キャリア名）: #ahamo #楽天モバイル #LINEMO #povo #UQモバイル #ワイモバイル #mineo #IIJmio
サブ（テーマ別）: #格安スマホ #SIM比較 #乗り換え #MVNO #MNP #節約 #家計見直し #スマホ代
サブ（用途別）: #子供スマホ #シニアスマホ #デュアルSIM #海外旅行SIM

## コンテンツ方針
- 各キャリアの最新料金プラン比較
- 乗り換え手順の簡潔な解説
- 用途別（子供用・シニア用・副回線用・海外旅行用）のおすすめ
- 実際の速度測定データや使用感
- 最新のキャンペーン・割引情報
- 月額いくら節約できるかの具体的な数字（ビフォーアフター形式）
- 「まだ大手キャリアに払ってるの？」系の問題提起

## 禁止事項
- 特定キャリアの過度な宣伝（比較サイトとしての中立性を保つ）
- 未確認の速度や品質に関する断定
- 景品表示法に抵触する表現（「No.1」等の根拠なし主張）
- ブログ運営ネタ（PV・SEO等）
...
def get_latest_articles() -> list[dict]:
    """Get latest articles from WordPress for promotion tweets."""
    try:
        with open(SECRETS_FILE, "r", encoding="utf-8") as f:
            secrets = json.load(f)
...
def main():
    parser = argparse.ArgumentParser(description="X自動投稿 — sim-hikaku.online")
    ...
    print(f"[{datetime.now().isoformat()}] X自動投稿開始 (slot: {args.slot}, site: sim-hikaku.online)")
    ...
    if not X_CREDS_FILE.exists():
        ...
        print("sim-hikaku.online用のXアカウントを設定してください")
    ...
    if tweet_id:
        ...
        notify_discord(f"[sim-hikaku] X投稿完了 ({args.slot})\n\n{text}\n\nhttps://x.com/{account}/status/{tweet_id}")
        print("完了")
    else:
        notify_discord(f"[sim-hikaku] X投稿失敗 ({args.slot})\n\n{text}")
        sys.exit(1)
