...
SITE_DIR = SCRIPT_DIR.parent.parent  # nambei-oyaji.com/
CONFIG_DIR = SITE_DIR / "config"
REPORTS_DIR = SITE_DIR / "outputs" / "reports"
REPORTS_DIR.mkdir(parents=True, exist_ok=True)

SITE_NAME = "南米おやじの海外生活ラボ"
...
def fetch_gsc_page_data(days=14):
    ...
    # nambei-oyaji.comはcredentials_fileに"config/"プレフィックスがある場合を考慮
...
def generate_optimized_title(old_title, top_queries, position):
    """検索クエリに基づいてタイトルを最適化（nambei-oyaji.com向け）"""
    new_title = old_title
    ...
    # 年号がなければ追加（移住・生活・費用系）
    ctr_keywords = ["移住", "費用", "生活費", "永住権", "ビザ", "失敗", "注意点",
                     "おすすめ", "比較", "副業", "稼ぐ", "在住"]
    ...
    # CTRパワーワード追加（パラグアイ移住系）
    if "在住者が語る" not in new_title and "在住" not in new_title:
        if "パラグアイ" in new_title and "｜" in new_title:
            parts = new_title.split("｜", 1)
            new_title = f"{parts[0]}｜在住者が本音で解説"
    ...
    # 数字を強調
    has_number = bool(re.search(r"\d+選|\d+つ|\d+個|\d+ステップ", new_title))
    if not has_number and "注意点" in new_title and "プロ" not in new_title:
        pass  # 既に「7選」等が付いていることが多いので無理に追加しない
...
def generate_meta_description(title, top_queries, slug):
    """検索クエリに基づいてメタディスクリプションを生成（nambei-oyaji.com向け）"""
    ...
    kw_phrases = []
    if any("費用" in kw for kw in query_keywords):
        kw_phrases.append("具体的な費用・生活費を公開")
    if any("失敗" in kw or "注意" in kw for kw in query_keywords):
        kw_phrases.append("在住者だから分かる落とし穴を解説")
    if any("ビザ" in kw or "永住権" in kw for kw in query_keywords):
        kw_phrases.append("最新のビザ・永住権情報を掲載")
    if any("副業" in kw or "仕事" in kw for kw in query_keywords):
        kw_phrases.append("海外から実践した収益実績も公開")

    if kw_phrases:
        desc += "・".join(kw_phrases[:2]) + "。"

    desc += "パラグアイ在住者が本音で解説します。"
