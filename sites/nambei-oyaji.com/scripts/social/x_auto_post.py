...
COOLDOWN_MINUTES = 15  # 前回投稿からの最小間隔（分）
...
## エンゲージメント向上テクニック（毎回いずれかを使う）
- **問いかけ型**: 「〜って知ってた？」「〜だと思う？」で読者に語りかける
- **ビフォーアフター型**: 「日本にいた頃→パラグアイに来てから」の対比で驚きを生む
- **ぶっちゃけ型**: 「正直に言うと〜」「誰も教えてくれなかったけど〜」で本音感を出す
- **数字ドン型**: 冒頭に衝撃的な数字を置いて止める「月3万円。これがインター校の学費」
- **失敗談型**: 「やらかした」「想定外だった」系の共感ネタ
- **あるある否定型**: 「"海外移住は金持ちの道楽"って言われるけど、実際は〜」で常識を覆す

## 投稿フォーマット（ランダムに選ぶ）
A. 短文インパクト型: 1〜2文で衝撃的な事実をドン→補足1文→ハッシュタグ
B. ストーリー型: 「今日〜があった」から始まる小さなエピソード→気づき→ハッシュタグ
C. 比較型: 「日本では○○。パラグアイでは△△。」の対比→感想→ハッシュタグ
D. 問いかけ型: 質問で始める→自分なりの答え→ハッシュタグ
E. リスト型: 「パラグアイ移住して変わったこと3つ」→箇条書き→ハッシュタグ
...
## 使い古されたネタ（避けること）
以下は過去に何度も投稿済み。同じ切り口・同じ結論の投稿は禁止:
- 「インター月3万円、日本なら15-20万円」（学費比較の直球表現）
- 「時差12時間を武器に」「子どもを寝かしつけてから仕事」
- 「花粉ゼロ、薬代年2万円がゼロに」（そのまま言う形）
- 「牛肉2kgで1,500-1,800円、アサード」（BBQ+値段のセット）
- 「冬でも最低気温12-14℃、コート不要」
これらのネタを使う場合は、必ず新しい角度・エピソード・感情を加えること。
...
def get_last_post_time() -> datetime | None:
    """Read last successful post timestamp from JSONL log file."""
    ...
    if not args.dry_run:
        ...
        if last_post:
            ...
            if elapsed < COOLDOWN_MINUTES:
                print(f"SKIP: Last post was {int(elapsed)}min ago (cooldown: {COOLDOWN_MINUTES}min). Aborting.")
                return
