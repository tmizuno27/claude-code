...
## 11. 正直なリスク評価（ZERO手動作業は可能か？）

### 100%自動化の現実

| 要素 | 自動化可能？ | 備考 |
|------|------------|------|
| ドメイン購入 | Yes（API経由） | Namecheap/Cloudflare API |
| ホスティング設定 | Yes | Vercel/Cloudflare Pages + Git push |
| コンテンツ生成 | Yes | Claude API + テンプレート |
| デプロイ | Yes | GitHub Actions → 自動 |
| SEO（サイトマップ等） | Yes | 自動生成 |
| Google Search Console登録 | 半自動 | 初回のみ手動 |
| AdSense/Mediavine申請 | **No** | **手動審査必須** |
| アフィリエイト申請 | **No** | **各ASPで手動申請** |
| Googleアルゴ変動への対応 | **No** | **判断が必要** |
| データソースAPI障害対応 | **No** | **監視+手動修正** |

### 結論: 「初期セットアップ後ZERO手動作業」は非現実的

**現実的なメンテナンス**: 安定稼働後も**週2-3時間**は必要。
- 月次: 低品質ページの特定・noindex化、データ更新確認
- 四半期: Googleアップデート対応、テンプレート改善
- 突発: API変更、デインデックス対応

### 最大のリスク（正直に）

1. **Google March 2024型アップデートの再来**: pSEOサイトが大量にデインデックスされた前例あり。回復に3-12ヶ月
2. **AI Overview（SGE）の拡大**: Google検索結果にAI回答が表示され、クリックスルー率が激減するリスク
3. **収益化の壁**: Mediavine（月5万セッション必要）に到達するまで6-12ヶ月、その間は低RPMのAdSenseのみ
4. **競合の激化**: pSEOツール（Byword, Seomatic等）の普及で参入障壁が下がり、同じニッチに大量参入
5. **「放置」は劣化する**: データが古くなる → 品質低下 → ランキング下落 → 収益減少のスパイラル

### 4モデルの比較評価

| モデル | 自動化度 | 初期コスト | 収益までの期間 | 月収ポテンシャル | リスク |
|--------|---------|-----------|---------------|----------------|--------|
| **AdSense/Mediavine pSEO** | ★★★★☆ | $100-350/月 | 6-12ヶ月 | $1,000-10,000 | 中（アルゴ変動） |
| **アフィリエイト比較サイト** | ★★★☆☆ | $100-350/月 | 8-18ヶ月 | $3,000-30,000 | 高（ASP審査+アルゴ） |
| **無料ツールサイト（計算機等）** | ★★★★★ | $50-100/月 | 3-6ヶ月 | $500-5,000 | 低（実用価値が高い） |
| **ディレクトリサイト** | ★★★★☆ | $50-200/月 | 6-12ヶ月 | $500-3,000 | 中（差別化が困難） |

### 最も「放置に近い」推奨モデル

**無料ツールサイト（計算機・コンバーター）が最適**:
- 理由1: ツールの実用価値がありGoogleに評価されやすい（情報ゲイン高い）
- 理由2: データ更新が不要（計算ロジックは変わらない）
- 理由3: pSEOペナルティのリスクが最も低い（各ページが固有の機能を持つ）
- 理由4: AdSense RPMが比較的高い（ユーザーが能動的にサイトを使う=エンゲージメント高い）
- 理由5: 競合は多いが、ニッチな計算機（例：「パラグアイ生活費計算機」「海外送金手数料比較」）なら空白あり

---

*リサーチ日: 2026-03-15（追加リサーチで更新）*
