...
def fetch_all_items(session, endpoint):
    ...
    per_page = 50
    while True:
        url = f"https://otona-match.com/?rest_route=/wp/v2/{endpoint}&per_page={per_page}&page={page}&status=publish,draft,private"
...
def main():
    parser = argparse.ArgumentParser(description="WPコンテンツバックアップ")
    parser.add_argument("--pages", action="store_true", help="固定ページも含む")
    args = parser.parse_args()

    logger.info("========== WPバックアップ 開始 ==========")
