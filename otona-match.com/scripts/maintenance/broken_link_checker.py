...
def fetch_published_posts(session, limit=100):
    ...
    per_page = 50
    while True:
        url = f"https://otona-match.com/?rest_route=/wp/v2/posts&status=publish&per_page={per_page}&page={page}"
...
def extract_urls(html_content):
    ...
    external = []
    for url in urls:
        if url.startswith(("http://", "https://")) and "otona-match.com" not in url:
            external.append(url)
    # 内部リンクも別途チェック
    internal = []
    for url in urls:
        if "otona-match.com" in url:
            internal.append(url)
