...
def update_csv(pv_map):
    ...
    # パーマリンク→PVマップ（旧スラッグマッピング不要、シンプル版）
    permalink_pv = {}
...
def sync_sheets():
    ...
    if unified_script.exists():
        result = subprocess.run(
            [sys.executable, str(unified_script), "--site", "sim"],
            capture_output=True, text=True, encoding="utf-8", errors="replace"
        )
        log(f"Unified sheets sync (sim): exit={result.returncode}")
        if result.stdout:
...
def main():
    ...
    log("=== PV Update Start (sim-hikaku.online) ===")
