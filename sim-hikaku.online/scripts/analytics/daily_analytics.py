...
def fetch_wordpress_stats(settings, secrets):
    ...
    rest_api_url = "https://sim-hikaku.online/wp-json/wp/v2"
...
# レポート生成
# =====================================================
DAILY_REPORT_PROMPT = """あなたは「SIM比較オンライン」(sim-hikaku.online) のアクセス分析アドバイザーです。
...
## サイトの特徴
- 格安SIM比較サイト（格安SIM比較、料金プラン、速度比較、用途別おすすめ）
- 収益源: 格安SIMアフィリエイト（アクセストレード、バリューコマース、A8.net）
- ターゲット: 格安SIM乗り換え検討中の日本人

## 出力形式
Markdownで出力。見出し・箇条書き・表を適切に使用。"""


def generate_daily_report(ga4_data, sc_data, wp_data, start_date, end_date, compare_data=None):
...
def generate_template_report(ga4_data, sc_data, wp_data, start_date, end_date):
    ...
    sections.append("---")
    sections.append("*このレポートは daily_analytics.py (sim-hikaku.online) により自動生成されました*")
...
def main():
    parser = argparse.ArgumentParser(description="日次アクセス分析レポート (sim-hikaku.online)")
    ...
    logger.info(f"=== 日次アクセス分析開始 (sim-hikaku.online, days={args.days}) ===")
    ...
    report = generate_daily_report(
        ga4_data, sc_data, wp_data, start_date, end_date, compare_data
    )
