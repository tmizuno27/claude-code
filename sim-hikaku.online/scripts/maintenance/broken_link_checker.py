...
def fetch_published_posts(session, limit=100):
    ...
    per_page = 50
    while True:
        url = f"https://sim-hikaku.online/wp-json/wp/v2/posts?status=publish&per_page={per_page}&page={page}"
...
def extract_urls(html_content):
    ...
    external = []
    for url in urls:
        if url.startswith(("http://", "https://")) and "sim-hikaku.online" not in url:
            external.append(url)
    internal = []
    for url in urls:
        if "sim-hikaku.online" in url:
            internal.append(url)
...
def main():
    parser = argparse.ArgumentParser(description="リンク切れチェッカー (sim-hikaku.online)")
    ...
    logger.info("========== リンク切れチェック 開始 (sim-hikaku.online) ==========")
    ...
    for post in posts:
        ...
        for url in all_urls:
            ...
            if error or (status_code and status_code >= 400):
                ...
                logger.warning(f"  NG [{status_code or 'ERR'}] {url} (in: {title})")
            else:
                logger.debug(f"  OK [{status_code}] {url}")
    ...
    if not args.dry_run and affiliate_urls:
        ...
        for url in affiliate_urls:
            ...
            if error or (status_code and status_code >= 400):
                ...
                logger.warning(f"  NG アフィリ [{status_code or 'ERR'}] {url}")
            time.sleep(0.5)
    ...
    if not args.no_discord:
        ...
        if broken_links:
            summary += f"**{len(broken_links)}件のリンク切れ検出**\n"
