#!/usr/bin/env python3
"""サイト死活監視 — nambei-oyaji.com のダウン検知 → Discord通知
...
CHECK_ENDPOINTS = [
    "/",                          # トップページ
    "/wp-json/wp/v2/posts?per_page=1",  # REST API
]
TIMEOUT = 20
...
def check_endpoint(url):
    ...
    try:
        ...
        resp = requests.get(url, timeout=TIMEOUT, headers={
            "User-Agent": "UptimeMonitor/1.0 nambei-oyaji.com"
        })
...
def main():
    parser = argparse.ArgumentParser(description="サイト死活監視")
    ...
    for endpoint in CHECK_ENDPOINTS:
        ...
        if ok:
            logger.info(f"  ✓ {endpoint} [{status_code}] {elapsed_ms}ms")
        else:
            logger.warning(f"  ✗ {endpoint} [{status_code or 'ERR'}] {error}")
            all_ok = False
