...
データソース:
  ...
  4. WordPress Jetpack Stats（フォールバック）— GA4未設定時
...
# =====================================================
# データソース 3: WordPress REST API（フォールバック）
# =====================================================
...
# レポート生成
# =====================================================
DAILY_REPORT_PROMPT = """あなたは「南米おやじの海外生活ラボ」(nambei-oyaji.com) のアクセス分析アドバイザーです。
...
## ブログの特徴
- パラグアイ在住の日本人が運営する海外生活ブログ
- 収益源: アフィリエイト（VPN、オンラインスクール、クレジットカード、サーバー）
- コンテンツ柱: パラグアイ生活（集客） + 海外からの仕事（収益化）
- ターゲット: 海外移住を検討する30-40代日本人
...
def generate_template_report(ga4_data, sc_data, wp_data, start_date, end_date):
    ...
    if ga4_data and ga4_data.get("status") == "ok":
        ...
        sections.append("⚠️ GA4未設定。`config/settings.json` の `property_id` を設定してください。\n")
    ...
    if sc_data and sc_data.get("status") == "ok":
        ...
        sections.append("⚠️ Search Console未設定。\n")
...
def main():
    ...
    # 分析期間
    end_date = datetime.now() - timedelta(days=1)  # 前日まで
