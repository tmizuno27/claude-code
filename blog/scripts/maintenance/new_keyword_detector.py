...
def fetch_search_console_data(secrets, days=7):
    ...
    try:
        response = service.searchanalytics().query(
            siteUrl="https://nambei-oyaji.com/",
            body=body,
        ).execute()
...
def main():
    ...
    if not args.no_discord and new_keywords:
        ...
        for kw in new_keywords[:7]:
            emoji = "🔥" if kw["potential"] == "high" else "📊" if kw["potential"] == "medium" else "📝"
            summary += f"{emoji} **{kw['query']}** — {kw['clicks']}click, pos:{kw['position']}\n"
