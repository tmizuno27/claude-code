...
function formatTable(table) {
    ...
    const dataLines = table.dataRows.map((row) => '| ' + row.map((cell, i) => padCell(cell, widths[i], aligns[i])).join(' | ') + ' |');
    return [headerLine, sepLine, ...dataLines].join('\n');
}
// ---------------------------------------------------------------------------
// Find table range around cursor
// ---------------------------------------------------------------------------
function findTableRange(doc, pos) {
...
function getTableAtCursor(editor) {
...
function getCursorColumnIndex(editor) {
...
function getCursorRowIndex(editor, tableStartLine) {
...
function rowsToTable(rows) {
    ...
    const aligns = Array.from({ length: colCount }, () => 'none');
    const dataRows = rows.slice(1).map((r) => Array.from({ length: colCount }, (_, i) => r[i] ?? ''));
    return { headerRow: header, separatorAligns: aligns, dataRows };
}
// ---------------------------------------------------------------------------
// Commands
// ---------------------------------------------------------------------------
async function cmdFormat(editor) {
...
async function cmdCsvToTable(editor) {
...
async function cmdTableToCsv(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        vscode.window.showWarningMessage('No Markdown table found at cursor.');
        return;
...
async function cmdImportCsvFile(editor) {
...
// Column operations
// ---------------------------------------------------------------------------
async function cmdAddColumnLeft(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdAddColumnRight(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdRemoveColumn(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdMoveColumnLeft(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdMoveColumnRight(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
    const col = getCursorColumnIndex(editor);
    const { table, range } = result;
    if (col >= table.headerRow.length - 1) {
        return;
    }
    swap(table.headerRow, col, col + 1);
    swap(table.separatorAligns, col, col + 1);
...
async function cmdRenameHeader(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
// Row operations
// ---------------------------------------------------------------------------
async function cmdAddRowAbove(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdAddRowBelow(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdDeleteRow(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdMoveRowUp(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
...
async function cmdMoveRowDown(editor) {
    const result = getTableAtCursor(editor);
    if (!result) {
        return vscode.window.showWarningMessage('No Markdown table found at cursor.');
    }
    const { table, range } = result;
    const rowIdx = getCursorRowIndex(editor, range.start.line);
    if (rowIdx < 0 || rowIdx >= table.dataRows.length - 1) {
        return;
    }
    swap(table.dataRows, rowIdx, rowIdx + 1);
    await editor.edit((eb) => eb.replace(range, formatTable(table)));
...
function formatAllTables(doc) {
    ...
    while (i < doc.lineCount) {
        ...
        const start = i;
        while (i < doc.lineCount && doc.lineAt(i).text.trim().includes('|')) {
            i++;
        }
        const end = i - 1;
        if (end - start < 1) {
...
function activate(context) {
    ...
    // Core commands
    reg('markdownTablePro.format', cmdFormat), reg('markdownTablePro.sort', cmdSort), reg('markdownTablePro.csvToTable', cmdCsvToTable), reg('markdownTablePro.tableToCsv', cmdTableToCsv), reg('markdownTablePro.importCsvFile', cmdImportCsvFile), 
    // Column operations
    reg('markdownTablePro.addColumnLeft', cmdAddColumnLeft), reg('markdownTablePro.addColumnRight', cmdAddColumnRight), reg('markdownTablePro.removeColumn', cmdRemoveColumn), reg('markdownTablePro.moveColumnLeft', cmdMoveColumnLeft), reg('markdownTablePro.moveColumnRight', cmdMoveColumnRight), reg('markdownTablePro.renameHeader', cmdRenameHeader), 
    // Row operations
    reg('markdownTablePro.addRowAbove', cmdAddRowAbove), reg('markdownTablePro.addRowBelow', cmdAddRowBelow), reg('markdownTablePro.deleteRow', cmdDeleteRow), reg('markdownTablePro.moveRowUp', cmdMoveRowUp), reg('markdownTablePro.moveRowDown', cmdMoveRowDown), 
    // Format on save
