...
## ブログデザイン確定版（2026-03-14 ゴールデンステート）
- **詳細**: [blog-design-golden-state.md](blog-design-golden-state.md)
- PC版・スマホ版ともにユーザーが「過去最高」と評価した状態
- 復元用リファレンス＆新サイト作成時のテンプレートとして保存
- Apple風カラー・frosted glassヘッダー・6カテゴリーカード・IntersectionObserverアニメーション

## ブログ記事フォーマット統一ルール
- **詳細**: [blog-article-format.md](blog-article-format.md)
- ヘッダー: ロゴ左+ナビ右寄せ、サイドバー: Apple風カード（プロフィール/検索/カテゴリ/新着/目次）
- 禁止: 記事内プロフィール、目次重複、heroヘッダー、SNSボタン、巨大SVG
- 新規記事投稿時はexcerpt必須、Puppeteerでフォーマット確認

## ブログ記事の視覚的強調ルール
- **詳細**: [blog-visual-emphasis.md](blog-visual-emphasis.md)
- 太字+カラーマーカー（黄=結論/数字、ピンク=注意、青=メリット）で重要部分を強調
- 1記事あたりマーカー5-10箇所、読者を飽きさせない視覚的リズム
- **範囲表現は全体を`<strong>`で囲む**（`<strong>月15万円〜20万円</strong>` ← 正しい）
- 全記事共通の永続ルール
...
- **対象**: ステータス変更、公開日追加、タイトル修正、文字数更新、アフィリ数変更、内部リンク数変更、備考追加など全項目
- **WordPress投稿時**: WP投稿IDとURLも記録に反映する
- **Google Sheetsとの連携**: CSV更新後、`create_article_sheet.py` も実行してスプレッドシートを同期する
...
- **投稿頻度**: 週3回（2026-03-12に週2→週3へ変更）、X投稿は1日3回（質重視）
...
- **温暖な気候**: 一年中温暖で晴れが多い
- **子供の教育**: 小学生の娘2人がインターナショナルスクールに通学。英語+スペイン語のバイリンガル教育。日本語も学ばせてトリリンガルを目指す
...
- 日本語でコミュニケーション
- **フォルダ別CLAUDE.md**: 各事業フォルダ（sedori/, freelance/, ebay/等）の作業が複雑化してきたら、フォルダ固有のCLAUDE.mdを作成する。判断基準：固有ルールが3つ以上、または独自の技術スタック・命名規則がある場合
- **居住地**: パラグアイ（タイムゾーン: America/Asuncion, 通年UTC-3 ※2024年10月にサマータイム廃止）
