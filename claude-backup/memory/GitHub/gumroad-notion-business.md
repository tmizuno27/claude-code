---
name: Gumroad Notionテンプレート販売事業
description: Gumroad上でNotionテンプレート10本+バンドルを販売する完全自動デジタル商品事業
type: project
---

Notionテンプレート10本をGumroadで販売する完全自動事業（2026-03-16構築）

**Why:** 収入の柱を増やすため。制作後は完全放置で自動配信・自動入金。

**How to apply:** `claude-code/gumroad-notion/` 配下に全ファイル格納。テンプレート設計書・Gumroad出品テキスト・Xマーケティング投稿を完備。

## 商品ラインナップ
1. Freelance Business OS ($19)
2. Content Creator Dashboard ($15)
3. Student Study Hub ($9)
4. Life OS — Second Brain ($19)
5. Small Business CRM ($19)
6. Side Hustle Tracker ($15)
7. Social Media Content Planner ($12)
8. Job Search Tracker ($9)
9. Book & Learning Tracker ($12)
10. Digital Products Business OS ($15)
11. Ultimate Bundle — 全10本セット ($49)

## ステータス（2026-03-16更新）
- 設計書・出品テキスト・マーケティング素材: 完成
- Notion上でのテンプレート実構築: **完了**（10本全てNotion APIで自動作成済み）
- 自動構築スクリプト: `scripts/create_all_templates.py`, `scripts/share_templates.py`
- 親ページID: 31be2be6f3228050ace6e3e25ddf4a00
- Gumroad出品: **未着手**（アカウント開設が必要）
- 次ステップ: Notion共有ON → Gumroadアカウント開設 → 出品テキストコピペ → 公開
- 収益目標: 月$300〜$2,000（6ヶ月後）
